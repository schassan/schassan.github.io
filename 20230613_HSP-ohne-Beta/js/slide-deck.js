...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (this.config_.presenters) {
    ...
    if (presenters.length == 1) {
      ...
      var mail = p.mail ? '<span>mail</span><a href="mailto:' + p.mail + '">' + p.mail + '</a>' : '';
      ...
      var orcid = p.orcid ? '<span>orcid</span>' + '<a href="http://orcid.org/' + p.orcid + '">' + p.orcid.replace(/https?:\/\//, '') + '</a>' : '';

      var github = p.github ? '<span>github</span><a href="' + p.github + '">' + p.github.replace(/https?:\/\//, '') + '</a>' : '';

      var html2 = [gplus, mail, twitter, www, orcid, github].join('<br>');
